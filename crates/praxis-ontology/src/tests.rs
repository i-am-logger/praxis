...
fn test_ontology_check() {
    crate::validate::check_ontology::<TrafficLightOntology>().unwrap();
}
...
/// A simple proposition: is a number positive?
#[derive(Debug)]
struct IsPositive;
...
fn test_anyof() {
    ...
    assert!(either.evaluate(&-4).is_satisfied()); // not positive but even
    assert!(either.evaluate(&3).is_satisfied()); // positive but not even
    assert!(
...
fn test_implies() {
    ...
    assert!(if_pos_then_even.evaluate(&4).is_satisfied()); // pos AND even
    assert!(!if_pos_then_even.evaluate(&3).is_satisfied()); // pos but NOT even
    assert!(if_pos_then_even.evaluate(&-3).is_satisfied()); // not pos → vacuously true
}
