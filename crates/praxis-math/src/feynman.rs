...
impl Amplitude {
    ...
    pub fn from_phase(phase: f64) -> Self {
        Self { real: phase.cos(), imag: phase.sin() }
    }

    pub fn add(&self, other: &Amplitude) -> Amplitude {
        Amplitude { real: self.real + other.real, imag: self.imag + other.imag }
    }
...
impl PathIntegral {
    pub fn new(hbar: f64) -> Self {
        Self { paths: vec![], hbar, total_amplitude: Amplitude::zero() }
    }
...
impl Situation for PathIntegral {
    fn describe(&self) -> String {
        format!("{} paths, amplitude=({:.6},{:.6}), P={:.6}",
            self.paths.len(), self.total_amplitude.real, self.total_amplitude.imag, self.probability())
    }
...
impl Action for FeynmanAction {
    ...
    fn describe(&self) -> String {
        match self {
            FeynmanAction::AddPath { path } => format!("add path (S={:.6}, {} points)", path.action, path.points.len()),
...
impl Precondition<FeynmanAction> for PhaseConsistency {
    fn check(&self, pi: &PathIntegral, action: &FeynmanAction) -> PreconditionResult {
        if let FeynmanAction::AddPath { path } = action {
            if pi.hbar <= 0.0 {
                return PreconditionResult::violated("phase_consistency", "ℏ must be positive", &pi.describe(), &action.describe());
            }
            let phase = path.action / pi.hbar;
            if phase.is_nan() || phase.is_infinite() {
                return PreconditionResult::violated("phase_consistency", "phase is NaN/infinite", &pi.describe(), &action.describe());
            }
    ...
    fn describe(&self) -> &str { "e^(iS/ℏ) must be well-defined" }
}
...
pub fn new_path_integral(hbar: f64) -> Engine<FeynmanAction> {
    Engine::new(PathIntegral::new(hbar), vec![Box::new(PhaseConsistency)], apply_feynman)
}
...
mod tests {
    ...
    #[test]
    fn test_single_path() {
        let path = Path { points: vec![0.0, 1.0], action: 1.0 };
        let e = new_path_integral(1.0)
            .next(FeynmanAction::AddPath { path }).unwrap();
    ...
    fn test_two_paths_interfere() {
        ...
        let p1 = Path { points: vec![0.0, 1.0], action: 0.0 };
        let p2 = Path { points: vec![0.0, 1.0], action: std::f64::consts::PI };
        let e = new_path_integral(1.0)
            .next(FeynmanAction::AddPath { path: p1 }).unwrap()
            .next(FeynmanAction::AddPath { path: p2 }).unwrap();
        // cos(0) + cos(π) = 1 + (-1) = 0
    ...
    fn test_constructive_interference() {
        ...
        let p1 = Path { points: vec![0.0, 1.0], action: 0.0 };
        let p2 = Path { points: vec![0.0, 1.0], action: 0.0 };
        let e = new_path_integral(1.0)
            .next(FeynmanAction::AddPath { path: p1 }).unwrap()
            .next(FeynmanAction::AddPath { path: p2 }).unwrap();
        // cos(0) + cos(0) = 2, |amp|² = 4
    ...
    #[test]
    fn test_reset() {
        let p = Path { points: vec![0.0], action: 1.0 };
        let e = new_path_integral(1.0)
            .next(FeynmanAction::AddPath { path: p }).unwrap()
            .next(FeynmanAction::Reset).unwrap();
