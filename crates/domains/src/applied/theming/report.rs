...
///
/// The HTML report is ontology-driven:
...
impl Default for ThemePalette {
    /// GitHub dark theme fallback (used when no palette provided).
    fn default() -> Self {
        Self {
            bg: "#0d1117".into(),
            card: "#161b22".into(),
...
impl ThemePalette {
    ...
    pub fn from_colors(colors: &std::collections::HashMap<String, String>) -> Self {
        ...
        Self {
            bg: get("base00", &defaults.bg),
            card: get("base01", &defaults.card),
            border: get("base02", &defaults.border),
            hover: get("base02", &defaults.hover),
            fg: get("base05", &defaults.fg),
            fg2: get("base03", &defaults.fg2),
            pass: get("base0B", &defaults.pass),
            fail: get("base08", &defaults.fail),
            warn: get("base09", &defaults.warn),
...
/// Escape a string for safe embedding in JSON inside HTML <script> tags.
...
/// Generate an ontology-driven HTML report with default theme.
pub fn to_html(results: &[ThemeResult], dataset_name: &str) -> String {
    to_html_themed(results, dataset_name, &ThemePalette::default())
}

/// Generate an ontology-driven HTML report themed by a palette.
///
...
pub fn to_html_themed(
    ...
    // Build report spec from ontology — drives encoding decisions
    let spec = ReportSpec::from_fields("theme-validation", theme_report_fields());
    let _warnings = spec.warnings(); // Empty for default (all optimal)

    // Collect unique schemes for filter buttons
    let mut schemes: Vec<String> = results.iter().map(|r| r.scheme.clone()).collect();
    schemes.sort();
    schemes.dedup();
    let scheme_buttons: String = schemes
        .iter()
        .map(|s| {
            format!(
                "  <button onclick=\"filt('scheme:{}',this)\">{}</button>\n",
                s, s
            )
        })
        .collect();

    // Encoding annotations from spec
    let encoding_desc: String = spec
        .assignments
        .iter()
        .map(|a| format!("{}: {:?} ({:?})", a.field.name, a.variable, a.geom))
        .collect::<Vec<_>>()
        .join(", ");
...
<header>
  ...
  <p>Ontology-driven axiom evaluation — <span id="ds"></span> · <span id="n"></span> themes</p>
  <p style="font-size:0.7rem;color:var(--fg2);margin-top:0.25rem">Spec: {encoding_desc}</p>
</header>
...
<div class="table-view" id="tbl"><table><thead><tr>
  ...
  <th onclick="sort('scheme')">Scheme</th>
...
function render() {{
  ...
  let r = [...D.results].filter(x => {{
    ...
    if (f.startsWith('scheme:')) return x.scheme===f.slice(7);
    return true;
  }});
  r.sort((a,b) => {{
    let va=a[s], vb=b[s];
  ...
  document.getElementById('grid').innerHTML = r.map(x => {{
    ...
    return `<div class="theme-card ${{cls}}" onclick="this.querySelector('.detail').classList.toggle('open')">
      <div class="name"><span>${{x.theme}}/${{x.variant}}</span><div class="badges">
        ...
        <span class="badge info">${{x.scheme}}</span>
        ${{brk}}
      </div></div>
  ...
  document.getElementById('tbody').innerHTML = r.map(x => {{
    ...
    return `<tr>
      <td>${{x.theme}}</td><td>${{x.variant}}</td><td>${{x.scheme}}</td><td>${{x.polarity}}</td>
...
mod tests {
    ...
    proptest! {
        ...
        fn prop_html_contains_all_themes(results in proptest::collection::vec(arb_theme_result(), 1..10)) {
            ...
            for r in &results {
                // Check the JSON data section specifically (\"theme\":\"name\"), not the HTML boilerplate
                let needle = format!("\"theme\": \"{}\"", r.theme);
                prop_assert!(html.contains(&needle), "missing theme in HTML JSON data: {}", r.theme);
            }
        }
