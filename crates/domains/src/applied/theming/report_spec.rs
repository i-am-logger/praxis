    ...
    pub fn from_fields(name: impl Into<String>, fields: Vec<DataField>) -> Self {
        let assignments = fields
            ...
            .map(|f| {
                ...
                let geom = suitable_geoms(f.level)
                    .first()
                    .copied()
                    .unwrap_or(GeomType::Bar);
                EncodingAssignment {
                    field: f,
                    variable,
                    geom,
...
pub fn theme_report_fields() -> Vec<DataField> {
    vec![
        DataField::new(
            "luminance",
            DataLevel::Ratio,
            "Relative luminance (L*) per slot",
        ),
        DataField::new(
            "color_role",
            DataLevel::Ordinal,
            "Color slot position (base00→base07)",
        ),
        DataField::new("pass_fail", DataLevel::Nominal, "Axiom pass or fail"),
        DataField::new(
            "scheme_type",
            DataLevel::Nominal,
            "Scheme family (base16/base24/vogix16)",
        ),
...
mod tests {
    ...
    fn test_best_encoding_ratio_is_position() {
        assert_eq!(
            best_encoding(DataLevel::Ratio),
            Some(VisualVariable::Position)
        );
    }
