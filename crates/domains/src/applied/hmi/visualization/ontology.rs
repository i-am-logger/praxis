...
pub fn suitable_geoms(level: DataLevel) -> Vec<GeomType> {
    match level {
        ...
        DataLevel::Ordinal => vec![
            GeomType::Bar,
            GeomType::Line,
            GeomType::Tile,
            GeomType::Boxplot,
        ],
        DataLevel::Interval => vec![
            GeomType::Line,
            GeomType::Bar,
            GeomType::Area,
            GeomType::Point,
...
mod tests {
    ...
    fn test_grammar_pipeline_order() {
        let order = PipelineOrder;
        assert!(
            order.get(&GrammarLayer::Data).unwrap() < order.get(&GrammarLayer::Aesthetics).unwrap()
        );
        assert!(
            order.get(&GrammarLayer::Aesthetics).unwrap() < order.get(&GrammarLayer::Geom).unwrap()
        );
        assert!(
            order.get(&GrammarLayer::Coord).unwrap() < order.get(&GrammarLayer::Facet).unwrap()
        );
    }
