...
proptest! {
    ...
    fn prop_trace_grows(from in arb_square(), to in arb_square()) {
        ...
        match result {
            ...
            Err((e, _)) => prop_assert_eq!(e.trace().entries.len(), 1),
        }
    }
    ...
    fn prop_violations_have_rule(from in arb_square(), to in arb_square()) {
        let engine = new_game();
        if let Err((_, violations)) = engine.next(ChessAction::new(from, to)) {
            for v in &violations {
...
// See crates/praxis-chess/games/*.pgn for authentic game records.
