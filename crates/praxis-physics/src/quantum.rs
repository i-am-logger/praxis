...
impl QuantumParticle {
    pub fn new(delta_x: f64, delta_p: f64) -> Result<Self, &'static str> {
        if delta_x <= 0.0 || delta_p <= 0.0 { return Err("uncertainties must be positive"); }
    ...
    pub fn minimum_uncertainty() -> Self {
        ...
        Self { delta_x: dx, delta_p: dx }
    }

    pub fn uncertainty_product(&self) -> f64 { self.delta_x * self.delta_p }
    pub fn heisenberg_holds(&self) -> bool { self.uncertainty_product() >= HBAR / 2.0 - 1e-40 }
}

impl Situation for QuantumParticle {
    fn describe(&self) -> String {
        format!("Δx={:.4e} Δp={:.4e} ΔxΔp={:.4e} ≥ ℏ/2={:.4e}",
            self.delta_x, self.delta_p, self.uncertainty_product(), HBAR / 2.0)
    }
...
impl Action for QuantumAction {
    ...
    fn describe(&self) -> String {
        match self {
            QuantumAction::MeasurePosition { new_delta_x } => format!("measure position (Δx→{:.4e})", new_delta_x),
            QuantumAction::MeasureMomentum { new_delta_p } => format!("measure momentum (Δp→{:.4e})", new_delta_p),
        }
    }
}
...
impl Precondition<QuantumAction> for HeisenbergUncertainty {
    fn check(&self, p: &QuantumParticle, a: &QuantumAction) -> PreconditionResult {
        let next = apply_quantum(p, a);
        if next.heisenberg_holds() {
            PreconditionResult::satisfied("heisenberg",
                &format!("ΔxΔp={:.4e} ≥ ℏ/2={:.4e}", next.uncertainty_product(), HBAR / 2.0))
        } else {
            PreconditionResult::violated("heisenberg",
                &format!("ΔxΔp={:.4e} < ℏ/2={:.4e}: uncertainty principle violated", next.uncertainty_product(), HBAR / 2.0),
                &p.describe(), &a.describe())
        }
    }
    fn describe(&self) -> &str { "ΔxΔp ≥ ℏ/2" }
}
...
impl Precondition<QuantumAction> for PositiveUncertainty {
    fn check(&self, p: &QuantumParticle, a: &QuantumAction) -> PreconditionResult {
        ...
        if valid {
            ...
            PreconditionResult::violated("positive_uncertainty", "uncertainty must be positive", &p.describe(), &a.describe())
        }
    }
    fn describe(&self) -> &str { "uncertainties must be positive" }
}

fn apply_quantum(p: &QuantumParticle, a: &QuantumAction) -> QuantumParticle {
    match a {
        QuantumAction::MeasurePosition { new_delta_x } => {
            // Reducing Δx increases Δp to maintain ΔxΔp ≥ ℏ/2
            let min_dp = HBAR / (2.0 * new_delta_x);
            QuantumParticle { delta_x: *new_delta_x, delta_p: min_dp.max(p.delta_p) }
        }
        QuantumAction::MeasureMomentum { new_delta_p } => {
            let min_dx = HBAR / (2.0 * new_delta_p);
            QuantumParticle { delta_x: min_dx.max(p.delta_x), delta_p: *new_delta_p }
        }
    }
}
...
pub fn new_particle(delta_x: f64, delta_p: f64) -> Result<Engine<QuantumAction>, &'static str> {
    ...
    Ok(Engine::new(p, vec![Box::new(PositiveUncertainty), Box::new(HeisenbergUncertainty)], apply_quantum))
}

pub fn new_minimum_uncertainty() -> Engine<QuantumAction> {
    Engine::new(
        QuantumParticle::minimum_uncertainty(),
        vec![Box::new(PositiveUncertainty), Box::new(HeisenbergUncertainty)],
        apply_quantum,
    )
}

/// Photon energy: E = hf.
pub fn photon_energy(frequency: f64) -> f64 { H * frequency }

/// De Broglie wavelength: λ = h/p.
pub fn de_broglie_wavelength(momentum: f64) -> f64 { H / momentum }

/// Hydrogen energy levels: E_n = -13.6/n² eV.
pub fn hydrogen_energy_level(n: u32) -> f64 { -13.6 / (n as f64 * n as f64) }
...
mod tests {
    ...
    fn test_measure_position_increases_momentum_uncertainty() {
        let e = new_minimum_uncertainty()
            .next(QuantumAction::MeasurePosition { new_delta_x: 1e-20 }).unwrap();
    ...
    fn test_zero_uncertainty_blocked() {
        assert!(new_minimum_uncertainty()
            .next(QuantumAction::MeasurePosition { new_delta_x: 0.0 }).is_err());
    }
