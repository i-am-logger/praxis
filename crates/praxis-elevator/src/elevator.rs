...
impl Elevator {
    pub fn new(id: usize, capacity: u32, num_floors: usize) -> Self {
        let _ = num_floors; // available for future bounds checking
        Self {
            id,
            floor: 0,
    ...
    /// Add a stop. Returns false if the stop violates direction commitment.
    ...
    pub fn move_one(&mut self, num_floors: usize) -> Result<(), &'static str> {
        if self.door == DoorState::Open {
            return Err("cannot move with doors open");
        }
        match self.direction {
            Direction::Up => {
                if self.floor + 1 >= num_floors {
