use crate::dispatch::{Dispatch, DispatchStrategy};
...
impl Building {
    ...
    pub fn dispatch(&mut self) -> Vec<(usize, Request)> {
        ...
        for &(eid, ref req) in &assignments {
            ...
            if elevator.direction == crate::elevator::Direction::Idle {
                if req.floor > elevator.floor
                    || (req.floor == elevator.floor && req.destination > elevator.floor)
                {
                    elevator.direction = crate::elevator::Direction::Up;
                } else {
                    elevator.direction = crate::elevator::Direction::Down;
                }
            }
        }
    ...
    pub fn step(&mut self) -> Vec<Event> {
        ...
        for elevator in &mut self.elevators {
            ...
            // Move one floor
            if elevator.move_one(self.num_floors).is_ok() {
