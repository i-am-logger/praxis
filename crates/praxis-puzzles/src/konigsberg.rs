...
impl Graph {
    ...
    pub fn konigsberg() -> Self {
        Self {
            ...
            edges: vec![
                ...
                (1, 2), (1, 2), // 2 bridges B-C
                (1, 3), (1, 3), // 2 bridges B-D   → B degree = 5
    ...
    pub fn degree(&self, node: usize) -> usize {
        self.edges.iter().filter(|(a, b)| *a == node || *b == node).count()
    }

    pub fn odd_degree_count(&self) -> usize {
        (0..self.nodes).filter(|&n| self.degree(n) % 2 != 0).count()
    }
...
impl State {
    pub fn new(graph: Graph, start: usize) -> Self {
        Self { graph, position: start, traversed: HashSet::new() }
    }
    ...
    pub fn available_edges(&self) -> Vec<(usize, usize, usize)> {
        self.graph.edges.iter().enumerate()
            .filter(|(i, (a, b))| !self.traversed.contains(i) && (*a == self.position || *b == self.position))
            .map(|(i, (a, b))| (i, *a, *b))
            .collect()
    }
}
...
impl Situation for State {
    fn describe(&self) -> String {
        format!("at node {} traversed={}/{}", self.position, self.traversed.len(), self.graph.edges.len())
    }
    fn is_terminal(&self) -> bool { self.all_traversed() }
}

/// Cross a bridge (by edge index).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CrossBridge { pub edge_index: usize }

impl Action for CrossBridge {
    type Sit = State;
    fn describe(&self) -> String { format!("cross bridge {}", self.edge_index) }
}
...
impl Precondition<CrossBridge> for ValidCrossing {
    fn check(&self, s: &State, a: &CrossBridge) -> PreconditionResult {
        if a.edge_index >= s.graph.edges.len() {
            return PreconditionResult::violated("valid_crossing", "edge index out of range", &s.describe(), &a.describe());
        }
        if s.traversed.contains(&a.edge_index) {
            return PreconditionResult::violated("valid_crossing", "bridge already crossed", &s.describe(), &a.describe());
        }
        let (a_node, b_node) = s.graph.edges[a.edge_index];
        if a_node != s.position && b_node != s.position {
            return PreconditionResult::violated("valid_crossing", "bridge not connected to current position", &s.describe(), &a.describe());
        }
        PreconditionResult::satisfied("valid_crossing", "valid bridge crossing")
    }
    fn describe(&self) -> &str { "must cross adjacent, untraversed bridge" }
}
...
pub fn new_konigsberg(start: usize) -> Engine<CrossBridge> {
    Engine::new(State::new(Graph::konigsberg(), start), vec![Box::new(ValidCrossing)], apply_crossing)
}

pub fn new_graph(graph: Graph, start: usize) -> Engine<CrossBridge> {
    Engine::new(State::new(graph, start), vec![Box::new(ValidCrossing)], apply_crossing)
}
...
mod tests {
    ...
    fn test_cant_reuse_bridge() {
        let e = new_konigsberg(0).next(CrossBridge { edge_index: 0 }).unwrap();
    ...
    fn test_konigsberg_exhaustive_impossibility() {
        ...
        fn try_all(state: &State, depth: usize, max_depth: usize) -> bool {
            if state.all_traversed() { return true; } // found a solution!
            ...
            for (idx, _, _) in state.available_edges() {
                ...
                if try_all(&next, depth + 1, max_depth) { return true; }
            }
            false
        }
        ...
        for start in 0..g.nodes {
            ...
            assert!(!try_all(&state, 0, g.edges.len()),
                "Euler path should be impossible from node {}", start);
        }
    }
