...
pub enum Bank { Left, Right }

impl Bank {
    pub fn opposite(&self) -> Bank {
        match self { Bank::Left => Bank::Right, Bank::Right => Bank::Left }
    }
}
...
impl State {
    pub fn initial() -> Self {
        Self { farmer: Bank::Left, wolf: Bank::Left, goat: Bank::Left, cabbage: Bank::Left }
    }
...
impl Situation for State {
    fn describe(&self) -> String {
        format!("F:{:?} W:{:?} G:{:?} C:{:?}", self.farmer, self.wolf, self.goat, self.cabbage)
    }
    fn is_terminal(&self) -> bool {
        self.farmer == Bank::Right && self.wolf == Bank::Right
            && self.goat == Bank::Right && self.cabbage == Bank::Right
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Crossing { Alone, WithWolf, WithGoat, WithCabbage }
...
impl Precondition<Crossing> for ItemWithFarmer {
    fn check(&self, s: &State, a: &Crossing) -> PreconditionResult {
        ...
        if bank == s.farmer {
            ...
            PreconditionResult::violated("item_with_farmer", "item on other bank", &s.describe(), &a.describe())
        }
    }
    fn describe(&self) -> &str { "item must be on farmer's bank" }
}
    ...
    fn check(&self, s: &State, a: &Crossing) -> PreconditionResult {
        ...
        if next.is_safe() {
            ...
            let reason = if next.wolf == next.goat && next.farmer != next.wolf {
                "wolf would eat goat" } else { "goat would eat cabbage" };
            PreconditionResult::violated("safe_result", reason, &s.describe(), &a.describe())
        }
    }
    fn describe(&self) -> &str { "result must be safe" }
}
...
pub fn new_puzzle() -> Engine<Crossing> {
    Engine::new(State::initial(), vec![Box::new(ItemWithFarmer), Box::new(SafeResult)], apply_crossing)
}
...
mod tests {
    ...
    fn arb_crossing() -> impl Strategy<Value = Crossing> {
        prop_oneof![Just(Crossing::Alone), Just(Crossing::WithWolf),
                     Just(Crossing::WithGoat), Just(Crossing::WithCabbage)]
    }

    #[test]
    fn test_known_solution() {
        let e = new_puzzle()
            .next(Crossing::WithGoat).unwrap()
            .next(Crossing::Alone).unwrap()
            .next(Crossing::WithWolf).unwrap()
            .next(Crossing::WithGoat).unwrap()
            .next(Crossing::WithCabbage).unwrap()
            .next(Crossing::Alone).unwrap()
            .next(Crossing::WithGoat).unwrap();
    ...
    fn test_cant_take_from_other_bank() {
        let e = new_puzzle().next(Crossing::WithGoat).unwrap().next(Crossing::Alone).unwrap();
