...
use praxis_category::{Category, Entity as CategoryEntity};
use praxis_engine::{Action, Precondition, Situation};
...
fn test_engine_next_invalid() {
    ...
    assert!(result.is_err());
    if let Err((engine, violations)) = result {
...
fn test_engine_trace_with_violation() {
    ...
    let engine = result.unwrap_err().0;
